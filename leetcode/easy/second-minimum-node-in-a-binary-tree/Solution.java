...
class Solution {
    ...
    public int findSecondMinimumValue(TreeNode root) {
        ...
        secondMinimumValue(root);
        return secondMinimum < Long.MAX_VALUE ? (int)secondMinimum : -1;   
    }
    
    private void secondMinimumValue(TreeNode root) {
        if (root != null) {
            if (firstMinimum < root.val && root.val < secondMinimum) {
                secondMinimum = root.val;
            } else if (firstMinimum == root.val){
                secondMinimumValue(root.left);
                secondMinimumValue(root.right);
            }
        }
    }   
}